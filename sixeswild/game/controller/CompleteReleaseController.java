...
/**
 * Controller that allows a player to complete a release level.
 * 
 *
 */
public class CompleteReleaseController {
	 ...
	 */
	public CompleteReleaseController(Level level, GameViewApplication app) {
		this.level = level;
		this.app = app;
	}
	
	/**
	 * Checks if all buckets have been given a six.
	 * @return true if all buckets filled, false otherwise. 
	 */
	public boolean hasWon() {
		...
		for (i = 0; i < 9; i++) {
			for (j = 0; j<9; j++) {
				...
				if (testSq.isBucket() && !testSq.getBucketFilled()) {
					return false;
				}
			}
		}
