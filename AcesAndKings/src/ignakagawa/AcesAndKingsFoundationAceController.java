...
public class AcesAndKingsFoundationAceController extends SolitaireReleasedAdapter{
	 ...
	 */
	public AcesAndKingsFoundationAceController(AcesAndKings theGame, PileView foundationAce, MultiDeck deck) {
		super(theGame);

		this.theGame = theGame;
		this.src = foundationAce;
		this.deck = deck;
	}
	...
	public void mousePressed(MouseEvent me) {
		...
		Pile foundationAcePile = (Pile) src.getModelElement();
		if (foundationAcePile.count() == 0) {
		...
		if (w != Container.getNothingBeingDragged()) {
			System.err.println ("AcePileController::mousePressed(): Unexpectedly encountered a Dragging Object during a Mouse press.");
			return;
		}
	...
	public void mouseReleased(MouseEvent me) {
		...
		Pile foundationAce = (Pile) src.getModelElement();
		...
		if (fromWidget instanceof RowView) {
			...
			m = new ReserveToAceFoundationMove(fromPile, theCard, foundationAce);
		}
			...
			m = new TableauToAceFoundationMove(tableauPile, theCard, foundationAce);
			...
			m = new WasteToAceFoundationMove(wastePile, theCard, foundationAce);
		}
		else if(theGame.isFoundationKingPile((Pile) fromWidget.getModelElement())) {
			Pile foundationKing = (Pile) fromWidget.getModelElement();
			m = new FoundationKingToFoundationAceMove(foundationKing, theCard, foundationAce);
		}
